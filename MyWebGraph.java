...
public class MyWebGraph {
	...
	public boolean addVertex(String s) {
		return vertexList.putIfAbsent(s, new WebVertex(s)) == null;
	}
	...
	public boolean addEdge(String s, String t) {
        boolean sb = vertexList.containsKey(s);
        boolean tb = vertexList.containsKey(t);
        if (sb && tb) {
    ...
    public ArrayList<String> getEdgesInto(String v) {
    	...
    	for (WebVertex w : vertexList.values()) {
            if (w.links.contains(v))
                edges.add(w.url);
        }
        return edges;
    } 
